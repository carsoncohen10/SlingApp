  ...
  async (event) => {
    ...
    for (const [messageId, messageData] of Object.entries(afterChatHistory)) {
      if (!beforeChatHistory[messageId]) {
        newMessages.push({ id: messageId, ...(messageData as any) });
      }
    }
    ...
    try {
      ...
      tokens.push(...tokenResults.filter(token => token !== null) as string[]);
 ...
 */
export const sendBetNotification = onDocumentCreated(
  "Communities/{communityId}/Bets/{betId}",
  async (event) => {
    ...
    try {
      ...
      const members = communityData.members as string[];
      const communityName = communityData.name || "Community";
 ...
 */
export const sendBetSettledNotification = onDocumentUpdated(
  "Bet/{betId}",
  async (event) => {
    ...
    try {
      ...
      for (const participantDoc of participantsSnapshot.docs) {
        ...
        const payload = {
          ...
          token: userDocs.find(doc => doc.id === userEmail)?.data()?.fcm_token,
        };
 ...
 */
export const sendBetVoidedNotification = onDocumentUpdated(
  "Bet/{betId}",
  async (event) => {
